...
Om aan de slag te gaan voor [!DNL Catalog Service] Adobe Commerce, zijn de volgende stappen vereist:
...
1. Werk pakketafhankelijkheden bij.

   ```bash
   composer update "magento/catalog-service"
   ```

1. Commit en push codewijzigingen voor de `composer.json` en `composer.lock` bestanden.
...
Gebruik deze methode om de [!DNL Catalog Service] voor een on-premises exemplaar te installeren.

1. Gebruik Composer om de module Catalog Service aan uw project toe te voegen:

   ```bash
   composer require magento/catalog-service --no-update
   ```

1. Werk afhankelijkheden bij en installeer de extensie:

   ```bash
   composer update  "magento/catalog-service"
   ```

1. Adobe Commerce upgraden:
...
Ga als volgt te werk om ervoor te zorgen dat de catalogusexport correct wordt uitgevoerd:

- [Controleer of cron-taken worden uitgevoerd](https://experienceleague.adobe.com/nl/docs/commerce-knowledge-base/kb/troubleshooting/miscellaneous/cron-readiness-check-issues).
...
Vanuit de Commerce Admin kunt u het synchronisatieproces bewaken met behulp van het [Data Management Dashboard](https://experienceleague.adobe.com/nl/docs/commerce-admin/systems/data-transfer/data-dashboard). Gebruik de [Commerce CLI](../data-export/data-export-cli-commands.md#troubleshooting) en logboeken om het proces te beheren en problemen op te lossen.

### Toegang tot de service

De [!DNL Catalog Service] GraphQL API is toegankelijk vanaf het ` https://catalog-service.adobe.io/graphql` eindpunt met behulp van POST-opdrachten via HTTPS.

In uw GraphQL-query&#39;s moet u meerdere HTTP-headers opgeven, waaronder de openbare API-sleutel die u hebt toegevoegd aan de Adobe Commerce Services Connector-configuratie in de Admin. Zie de documentatie van Storefront [Services GraphQL](https://developer.adobe.com/commerce/services/graphql/) voor meer informatie.
...
Voor meer informatie over [!DNL Catalog Service] gegevenssynchronisatie, zie het [ Dashboard van het Beheer van Gegevens ](https://experienceleague.adobe.com/nl/docs/commerce-admin/systems/data-transfer/data-dashboard).
