...
source-git-commit: b116634e2a1534f08d6f2c7962ff1223637f7a17
workflow-type: tm+mt
...
De volgende items worden op 27 april 2026 vrijgegeven voor productieomgevingen.
...
* Het `GET /V1/order-statuses` REST API-eindpunt voor het ophalen van alle geconfigureerde orderstatussen met hun statustoewijzingen toegevoegd. <!-- CEXT-6100 -->

* Oplossing voor een probleem dat ertoe leidde dat `custom_attributes` voor entiteiten zoals Order, Cart, Factuur, Credit Memo en Company niet in het REST-schema werd weergegeven. <!-- CCSAAS-4818 -->

* Oplossing duplicate message processing errors (`MessageLockException`) in the async bulk API consumer. <!-- CCSAAS-4805 -->

* Productkenmerken voor nummers worden nu gerenderd als van/naar bereikfilters in het productraster van [!DNL Commerce Admin] wanneer het kenmerk is ingeschakeld voor filteropties. <!-- ACCS-761 -->

* Probleem verholpen waarbij per e-mail voor het verlaten van het winkelwagentje geen productafbeeldingen werden weergegeven bij gebruik van [!DNL AEM Assets] . <!-- ACCS-798 -->

* Probleem verholpen waarbij een fout met de fout &#39;maximale uploadgrootte&#39; kon optreden wanneer bestanden, voorbeelden of koppelingen naar downloadbare producten werden toegevoegd. <!-- ACCS-813 -->

* Probleem verholpen waarbij het opslaan van een product dat is toegewezen aan meerdere gedeelde catalogi tot een fout kon leiden. <!-- ACCS-788 -->

* Probleem verholpen waarbij de query voor de ordergeschiedenis traag zou kunnen zijn en databasefouten met onvoldoende geheugen voor bedrijven met veel bestellingen zou veroorzaken. <!-- ACCS-808 -->

* Probleem verholpen waarbij de validatie van importbestanden kon mislukken. <!-- CCSAAS-4364 -->

* De configuratie van **[!UICONTROL Recently Viewed/Compared Products]** is verwijderd uit de sectie **[!UICONTROL Catalog]** in **[!UICONTROL Stores]** > _[!UICONTROL Settings]_>**[!UICONTROL Configuration]**, omdat deze niet wordt ondersteund in [!DNL Adobe Commerce as a Cloud Service] admin. <!-- ACCS-793 -->
...
De volgende items zijn op 16 april 2026 vrijgegeven voor productieomgevingen.

>[!BEGINSHADEBOX]

### Aangepaste aanhalingstekens implementeren met webhaken

`plugin.magento.out_of_process_totals_collector.api.get_total_modifications.execute` [&#x200B; webhaak &#x200B;](https://developer.adobe.com/commerce/extensibility/webhooks/) is nu beschikbaar in [!DNL Adobe Commerce as a Cloud Service]. Gebruik het om de wijzigingen van de douanecitaattotalen door uit-van-procesrekbaarheid uit te voeren. <!-- CEXT-5896 -->

### Regels voor herbruikbare e-mailherinneringen (experimenteel)

>[!IMPORTANT]
>
>Deze functie is experimenteel en moet worden ingeschakeld door contact op te nemen met de Adobe Commerce Customer Success Manager of door een ondersteuningsticket te maken.

[&#x200B; E-mail herinneringsregels &#x200B;](https://experienceleague.adobe.com/nl/docs/commerce-admin/marketing/communications/email-reminders/email-reminder-rules#rule-repeatability) steunt nu een facultatieve regelherbruikbaarheid die de zelfde regel toestaat om op een klant opnieuw toe te passen nadat de originele trekkervoorwaarde niet meer van toepassing is.

Bijvoorbeeld, als een klant een karretje verlaat, de aankoop voltooit, en later een nieuwe karretje verlaat, kan de regel opnieuw teweegbrengen. Zonder deze instelling wordt een klant die de oorspronkelijke trigger wist, permanent uitgesloten van toekomstige overeenkomsten met dezelfde regel.

### Het Transactierapport voor betalingsservices weergeven

Als u [[!DNL Payment Services] &#x200B;](https://experienceleague.adobe.com/nl/docs/commerce/payment-services/get-started/production) toegelaten hebt, is [&#x200B; Dashboard UI &#x200B;](../payment-services/payments-home.md) nu beschikbaar in [!DNL Commerce Admin], die toegang tot het [&#x200B; rapport van Transacties &#x200B;](../payment-services/reporting.md#transactions-report-view) verleent voor het bekijken en het beheren van betalingstransacties. <!-- PAY-6510 -->

### Verbeteringen en foutoplossingen

De volgende geselecteerde verbeteringen, optimalisaties en foutoplossingen zijn opgenomen in deze release:

* Oplossing voor een fout van 500 op de pagina Gedeelde catalogus toewijzen van bedrijven die kan optreden bij het gebruik van grote gedeelde catalogi met de beheerdersinterface van SDK. <!-- CCSAAS-4783 -->

* Probleem verholpen waarbij klanten van het bedrijf hun eigen bestellingen niet konden zien als deze bestellingen werden geplaatst voordat de klant aan het bedrijf werd toegewezen. <!-- ACCS-746 -->
...
De volgende items zijn op 1 april 2026 vrijgegeven voor productieomgevingen.
...
Een nieuw `numeric` [&#x200B; type van de productkenmerkinput &#x200B;](https://experienceleague.adobe.com/nl/docs/commerce-admin/catalog/product-attributes/attributes-input-types) staat verkopers toe om decimale attributen tot stand te brengen die negatieve waarden steunen. <!-- ACCS-600 -->
...
* Oplossing voor een fout die kon optreden wanneer een gastbestelling met orderopmerkingen werd geannuleerd. <!-- ACCS-674 -->

* Oplossing voor een fout die kon optreden wanneer een gegroepeerd product met veel bijbehorende items aan een aanvraaglijst werd toegevoegd. <!-- ACCS-627 -->
...
* Probleem verholpen waarbij de `setNegotiableQuoteShippingAddress` -mutatie niet handmatig ingevoerde adressen naar het adresboek van de klant opslaat, zelfs niet wanneer `save_in_address_book` was ingesteld op `true` . <!-- LYNX-1031 -->[!DNL Storefront Compatibility B2B Package]
...
U kunt [&#x200B; gebruikerstoegang &#x200B;](./user-management.md#add-users) aan individuele [!DNL Adobe Commerce as a Cloud Service] instanties in Admin Console nu toewijzen. <!-- CCSAAS-4337 -->
<!-- See PR #332 -->
...
* Verschillende verbeteringen op het gebied van prestaties en optimalisatie.
<!-- CCSAAS-4255 -->
<!-- CCSAAS-4233 -->
<!-- CCSAAS-4220 -->
<!-- CCSAAS-4252 -->
...
* Als een adres dat in het **configuratiescherm van de Adressen van de E-mail van de Opslag** wordt bepaald een waarde bevat die met `example.com` beëindigt, verzendt Commerce geen e-mails naar dit adres. In plaats daarvan logt het systeem aan dat de e-mail niet is verzonden.  <!-- CCSAAS-3533 -->
