...
- De onderzoeksadapter is [ afgekeurd ](release-notes.md#live-search-400) vanaf [!DNL Live Search] 4.0.0. De widget pagina met productlijsten (Product Listing Page, PLP) is de ondersteunde oplossing voor alle [!DNL Live Search] -implementaties die verdergaan. De adapter van het Onderzoek zal slechts op veiligheid betrekking hebbende updates ontvangen. Zie de [ migratiegids ](migrate-to-plp.md) voor informatie over het migreren aan PLP widget.
...
- Het maximale aantal resultaten per pagina is 100.
- Het is niet mogelijk om resultaten te filteren met behulp van een datumtype-kenmerk.

>[!NOTE]
>
>Sorteren op positie vereist dat een geldig `categoryPath` - of `categoryIds` -filter actief is. [Meer info](https://developer.adobe.com/commerce/webapi/graphql/schema/live-search/queries/product-search/#error-handling-for-categorypath-and-categoryids).
...
Raadpleeg de volgende artikelen in de knowledgebase voor hulp bij het oplossen van veelvoorkomende problemen in [!DNL Live Search] :

- [[!DNL Live Search]  niet gesynchroniseerde catalogus ](https://experienceleague.adobe.com/en/docs/commerce-knowledge-base/kb/troubleshooting/miscellaneous/live-search-catalog-data-sync) - verstrekt oplossingen voor wanneer de gegevens van de productcatalogus niet correct tussen uw opslag van Adobe Commerce en de Levende dienst van het Onderzoek synchroniseren. In dit artikel wordt beschreven hoe u de synchronisatiestatus kunt controleren, synchronisatiefouten kunt identificeren en problemen met gegevenssynchronisatie kunt oplossen.
- [[!DNL Live Search]  dashboard en het rangschikken van het onderzoeksresultaat is onjuist ](https://experienceleague.adobe.com/en/docs/commerce-knowledge-base/kb/troubleshooting/miscellaneous/live-search-dashboard-ranking-incorrect) - richt kwesties waar de onderzoeksresultaten of prestatiesmetriek die in het Levende dashboard van het Onderzoek worden getoond niet zoals verwacht. In dit artikel wordt uitgelegd hoe u problemen kunt oplossen met rangschikkingsverschillen en inconsistenties in dashboardgegevens.
- [[!DNL Live Search]  de facetten worden niet alfabetisch gesorteerd ](https://experienceleague.adobe.com/en/docs/commerce-knowledge-base/kb/troubleshooting/miscellaneous/live-search-facets-not-sorted) - lost de kwestie op waar de facetwaarden in een onverwachte orde eerder dan alfabetisch verschijnen. Dit artikel bevat stappen voor het configureren en corrigeren van gedrag voor het sorteren van facetten in uw winkel.
