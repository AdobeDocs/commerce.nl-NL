...
badgeSaas: label="Alleen SaaS" type="Positive" url="https://experienceleague.adobe.com/en/docs/commerce/user-guides/product-solutions" tooltip="Alleen van toepassing op Adobe Commerce Optimizer-projecten (door Adobe beheerde SaaS-infrastructuur)."
...
- **Geautomatiseerde kwestie opsporing** - Sites Optimizer scant voortdurend productcatalogi, onderzoekslogboeken, en aanbevelingsgegevens om problemen te identificeren die ontdekking beïnvloeden.
- **AI-gedreven aanbevelingen** - ontvang intelligente suggesties om ontdekte kwesties op te lossen.
- **Effect categoriseren** - De kwesties worden gecategoriseerd door bedrijfseffect (Onderzoek, Aanbevelingen, doorbladeren/Navigatie, de Kwaliteit van de Gegevens van het Product).
- **Dashboard die** rapporteert - de kwesties van de mening tendensen, hoogste beïnvloede producten of vragen, en verbeteringen in tijd.
...
De pagina Opportunity is ingedeeld in drie tabbladen waarmee u aanbevelingen voor optimalisatie kunt beheren:

- **Huidige (Actieve)** - Vertoningen onlangs ontdekte kansen die overzicht en actie vereisen. Dit zijn actieve problemen die gevolgen kunnen hebben voor de prestaties van uw site.
...
De workflow voor automatische detectie maakt gebruik van door AI aangedreven analyse om automatisch optimalisatiemogelijkheden in uw productcatalogus te identificeren. Dit geautomatiseerde scanproces bewaakt voortdurend uw productgegevens, zoeklogs en aanbevelingen om problemen op te sporen die van invloed kunnen zijn op de prestaties van de site, de SEO en de betrokkenheid van de klant.

### Hoe werkt het

Automatische detectie maakt gebruik van Adobe Experience Manager Sites Optimizer:

- **analyseert productpagina&#39;s** - het systeem onderzoekt de hoogste 200 pagina&#39;s en de filters voor de pagina&#39;s van het productdetail om optimalisatiedoelstellingen te identificeren.
- **de meta-gegevens van het Extraheren** - de markeringen van Meta (titels, beschrijvingen, H1 kopballen) worden gehaald uit elke pagina voor analyse.
- **produceer AI aanbevelingen** - de Getrekte gegevens worden verwerkt door het werkschema van Adobe AI om actionable optimaliseringssuggesties tot stand te brengen.
- **bevolkt kansen** - Auto-ontdekte suggesties verschijnen in het **Huidige (Actieve)** lusje voor uw overzicht.

### Vereiste

Voordat automatisch detecteren aanbevelingen kan genereren, moeten de catalogusgegevens zijn gesynchroniseerd en up-to-date om nauwkeurige aanbevelingen te kunnen doen.
...
De volgende lijsten de gesteunde opportuniteitstypes:
...
- Ontbrekende titel
- Lege titel
- Beschrijving te lang
- Beschrijving te kort
- Ontbrekende beschrijving
- Lege beschrijving
- Dubbele beschrijving
- H1 ontbreekt
- H1 dupliceren
- H1 te lang
...
>[!NOTE]
>
>Meerdere H1&#39;s op pagina worden momenteel niet ondersteund.
...
>[!IMPORTANT]
>
>Voor de functie voor automatisch repareren is een volledig opgenomen productcatalogus vereist. Als uw catalogus nog niet is opgenomen, kunt u nog steeds mogelijkheden bekijken en correcties handmatig implementeren met behulp van het aangeboden CSV-bestand. Merk op dat de handimplementaties niet in **Geoptimaliseerd (Gedaan)** tabel worden gevolgd.

### Een optimalisatie voor automatische correcties implementeren

Voer de volgende stappen uit om een door AI voorgestelde optimalisatie te implementeren:

1. Navigeer aan **leidt Resultaten** > **Kansen**.

1. In het **Huidige (Actieve)** lusje, herzie beschikbare optimaliseringssuggesties.
...
1. Klik **opstellen optimalisering** dan klik **&#x200B;**&#x200B;opstelt om het auto-moeilijke proces teweeg te brengen.
   ...
   - Hiermee maakt u een nieuwe cataloguslaag voor het product (als deze nog niet bestaat).
   - Werkt het desbetreffende kenmerk (zoals de titel van de meta, beschrijving of H1) bij op basis van de AI-aanbeveling.
   - Wijst de nieuwe laag als hoogste prioriteit (orde 1) in de catalogusmening toe.
   - Valideert de verandering door de dienst van de catalogusopslag.

1. Controleer de implementatiestatus. Het systeem werkt de status van de suggestie automatisch bij zodra de validatie is voltooid.

1. Zodra geoptimaliseerd, beweegt de suggestie zich aan **Geoptimaliseerd (Gedaan)** lusje met een statusindicator:
...
>[!NOTE]
>
>Automatische correctie ondersteunt optimalisatie van metagegevens voor sites in elke taal. Sites Optimizer analyseert productdetailpagina&#39;s in hun oorspronkelijke taal, genereert gelokaliseerde AI-aanbevelingen en maakt cataloguslagen op basis van de landinstelling van de bron die in de catalogusweergave is geconfigureerd.
...
Houd rekening met het volgende wanneer u automatisch repareren gebruikt:

- De status die voor elke suggestie wordt weergegeven, geeft de status aan die gold op het moment dat de worker voor automatisch repareren werd uitgevoerd. De status wordt niet dynamisch bijgewerkt als u de cataloguslagen daarna handmatig opnieuw rangschikt.

- Om ervoor te zorgen dat uw optimalisaties actief blijven, vermijd manueel veranderende prioriteiten van de cataloguslaag na het opstellen van auto-moeilijke fixaanbevelingen.

### Problemen oplossen

Als er geen optimalisatie lijkt te zijn toegepast op de winkel:

1. Controleer de statusindicator in **Geoptimaliseerde (Gedaan)** tabel.
1. Als u een waarschuwingspictogram ziet, verifieert u de prioriteitsinstellingen van de cataloguslaag.
1. Zorg ervoor dat de optimalisatielaag is ingesteld op volgorde 1 (hoogste prioriteit) in de catalogusweergave.
1. Controleer of de synchronisatie van catalogusgegevens actief en up-to-date is.
1. Toestaan dat wijzigingen worden doorgegeven. Zelfs met een behoorlijk gevormde laag bij orde 1, kunnen de veranderingen tijd vergen om op uw winkel, gelijkend op de vertraging te verschijnen wanneer het publiceren van nieuwe producten.
