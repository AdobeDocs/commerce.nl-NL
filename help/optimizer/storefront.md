...
1. **[pas de storefrontconfiguratie](#configure-your-storefront)** aan - werk het `config.json` dossier in uw bewaarplaats bij om met uw [!DNL Adobe Commerce Optimizer] instantie te verbinden.
...
1. Vervang de waarden van de plaatsaanduiding door de waarden voor de instantie. Zie [ Eerste vereisten ](#prerequisites).
   ...
   >[!NOTE]
   >
   >Om prijzenboekidentiteitskaart te vinden, controleer de [ details van de catalogusmening ](./setup/catalog-view.md) in [!DNL Adobe Commerce Optimizer] om de toegewezen prijsboeken te zien. Als geen prijzenboeken worden toegewezen, kunt u deze kopbal uit het configuratiedossier verwijderen. Voeg het terug wanneer een prijsboek aan de catalogusmening is toegewezen.
